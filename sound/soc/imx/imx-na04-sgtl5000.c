...
static void imx_3stack_init_dam(int ssi_port, int dai_port)
{
	...
	unsigned int td_ptcr = 0;					
	unsigned int td_pdcr = 0;
	int td_port=6;
	...
	__raw_writel(td_ptcr, DAM_PTCR(td_port));
	__raw_writel(ssi_pdcr, DAM_PDCR(ssi_port));
//	__raw_writel(dai_pdcr, DAM_PDCR(dai_port));
	__raw_writel(td_pdcr, DAM_PDCR(td_port));

	/* Internal network mode */						
	dai_pdcr |= 0x100;
	/* include RxD of ssi and td for ANDing */
	dai_pdcr |= 0xff;
	dai_pdcr &= ~(1<<(ssi_port-1));
	dai_pdcr &= ~(1<<(td_port-1));
	__raw_writel(dai_pdcr, DAM_PDCR(dai_port));


	td_pdcr=0;
	td_pdcr |= AUDMUX_PDCR_RXDSEL(ssi_port);
	__raw_writel(td_pdcr, DAM_PDCR(td_port));
